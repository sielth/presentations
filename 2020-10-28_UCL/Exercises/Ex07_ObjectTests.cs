...
namespace Exercises
{
    public class Ex07_ObjectTests
    {
        ...
        #region Helpers
        private AnnoyingClass GetResult() => new AnnoyingClass { id = new Random().Next(), name = "John" };

        private Person GetPerson() => new Person { Name = "John" };
        ...
        private Machine GetInstance() => new Machine { Name = "HAL", Id = 9000 };
        ...
        private static class ModelMapper
        {
            public static Model Map(ModelDto m) =>
                new Model { Created = m.Created + TimeSpan.FromMilliseconds(new Random().Next(1, 42)) };

            public static ModelDto Map(Model m) =>
                new ModelDto { Created = m.Created + TimeSpan.FromMilliseconds(new Random().Next(1, 42)) };
        }

        private static Model GetModel() => new Model { Created = 19.May(1978) };
        #endregion
    }
}
